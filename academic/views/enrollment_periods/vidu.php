    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <div class="alert alert-warning mt-2">
                ...
                Nếu vai trò <strong>Quản lý Đào tạo</strong> chưa có quyền <code>enrollment.manage_period</code>,
                bạn có thể cấp cho họ ngay bằng nút bên dưới hoặc thông qua
                <a href="<?= BASE_URL ?>/admin/permission_matrix.php" class="alert-link">ma trận phân quyền</a>.
            </div>
            <button id="grantEnrollPermBtn" class="btn btn-sm btn-outline-secondary mb-3">
                <i class="bi bi-shield-lock me-1"></i>Gán quyền quản lý đăng ký cho academic_admin
            </button>
            <script>
            document.getElementById('grantEnrollPermBtn')?.addEventListener('click', async function() {
                if (!confirm('Cấp quyền enrollment.manage_period cho role Quản lý Đào tạo?')) return;
                try {
                    const fd = new FormData();
                    fd.append('role_id', <?= $academicRoleId ?>);
                    fd.append('permission_ids[]', <?= $enrollPermId ?>);
