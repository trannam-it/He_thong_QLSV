<?php
/**
 * BaseAccountantController - Controller cơ sở cho vai trò Kế toán
 *
 * Flow phân quyền:
...
class BaseAccountantController
{
    protected mysqli           $conn;
    protected AccountantModel  $model;
    protected ?Auth            $auth;
    ...
    public function __construct(mysqli $conn)
    {
        ...
        // [LAYER 1] Router guard: chỉ accountant mới vào được module này
        AppRouter::guardModule(['accountant']);

        require_once __DIR__ . '/../models/AccountantModel.php';
        $this->model = new AccountantModel($conn);
    ...
    protected function requirePermission(string $permissionCode): void
    {
        if (!$this->auth->hasPermission($permissionCode)) {
            ...
            $back = defined('BASE_URL') ? BASE_URL . '/accountant/' : '/accountant/';
    ...
    protected function render(string $viewPath, array $data = []): void
    {
        ...
        $data['currentPage'] = AccountantRouter::getPageName();
        $data['auth']        = $this->auth;
    ...
    protected function abortError(string $message): void
    {
        ...
        echo '<div style="font-family:sans-serif;padding:60px;text-align:center;">'
            ...
            . '<a href="' . (defined('BASE_URL') ? BASE_URL : '') . '/accountant/" style="color:#fd7e14;">← Quay về Dashboard</a>'
            . '</div>';
        exit;
    }
}
